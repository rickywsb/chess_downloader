...
class TeacherChessApp:
    ...
    def find_username(self, name: str) -> str:
        ...
        # 3. 完整单词匹配 (避免部分匹配导致的混淆)
        # 只有当输入名字的所有单词都在学生姓名中时才匹配
        input_words = set(word.lower() for word in name.split() if len(word) > 1)
        if input_words:
            best_match = None
            best_score = 0
            
            for real_name, username in self.students.items():
                real_words = set(word.lower() for word in real_name.split() if len(word) > 1)
                
                # 计算匹配度：输入单词在学生姓名中的匹配比例
                if real_words:
                    matched_words = input_words & real_words
                    # 要求输入的所有单词都要匹配，并且匹配的单词数量要足够
                    if len(matched_words) == len(input_words) and len(matched_words) > 0:
                        # 优先选择姓名长度相近的匹配
                        score = len(matched_words) * 10 + (10 - abs(len(real_words) - len(input_words)))
                        if score > best_score:
                            best_score = score
                            best_match = username
        ...
        # 4. 单个名字的精确匹配 (如果输入只有一个单词)
        if len(name.split()) == 1 and len(name) > 2:
            for real_name, username in self.students.items():
                real_words = [word.lower() for word in real_name.split() if len(word) > 1]
                if lower in real_words:
                    return username
        
        # 5. 去除标点符号后的匹配
        clean_input = re.sub(r"[^\w]", "", lower)
        if clean_input:
            for real_name, username in self.students.items():
                clean_real = re.sub(r"[^\w]", "", real_name.lower())
                if clean_input == clean_real:
                    return username
